...
export class EventService {
  constructor(private readonly prisma: PrismaService , private readonly driveService: DriveService) {}
  ...
  async create (dto: CreateEventDto, userId:string, googleToken: string) {
    try{
      ...
      const folder = await this.driveService.createFolder(`Event_${dto.name}`, googleToken)
